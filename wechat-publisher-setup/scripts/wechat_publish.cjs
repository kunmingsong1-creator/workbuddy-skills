...
function defaultEnvPath() {
  return path.join(os.homedir(), '.openclaw', 'workspace-wechat-publisher', '.env');
}

function defaultTokenCachePath() {
  return path.join(os.homedir(), '.openclaw', 'workspace-wechat-publisher', '.access_token');
}

const ENV_FILE = process.env.WECHAT_ENV_FILE || defaultEnvPath();
const TOKEN_CACHE = process.env.WECHAT_TOKEN_CACHE || defaultTokenCachePath();
...
function loadEnv() {
  if (!fs.existsSync(ENV_FILE)) {
    ...
    console.error('   请先运行 /wechat-publisher-setup 配置微信 API 凭证');
    process.exit(1);
  }
