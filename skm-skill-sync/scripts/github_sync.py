...
使用方法：
  python ~/.workbuddy/skills/skill-sync/scripts/github_sync.py [push|pull|sync]
...
PAT 配置优先级（高→低）：
  ...
  3. Key Vault   key-vault skill（github-pat）

版本判断：比较本地 skill-sync/SKILL.md 中 version 字段 vs GitHub 最新 commit
"""
...
def load_token():
    ...
    try:
        import subprocess
        vault_script = os.path.expanduser("~/.workbuddy/skills/key-vault/scripts/vault.py")
...
def get_remote_version():
    """获取 GitHub 上 skill-sync/SKILL.md 的 version 字段"""
    try:
        content = api_req("GET", f"/contents/skill-sync/SKILL.md?ref={BRANCH}")
...
def get_local_version():
    """获取本地 skill-sync/SKILL.md 的 version 字段"""
    path = os.path.join(SKILLS_DIR, "skill-sync", "SKILL.md")
...
if mode in ("sync", "pull"):
    ...
    if remote_ver and local_ver:
        ...
        if cmp < 0:
            ...
            # 只拉取 skill-sync 自身（版本号在 skill-sync/SKILL.md 中）
            pull_from_github(target_skills=["skill-sync"])
        elif cmp > 0:
